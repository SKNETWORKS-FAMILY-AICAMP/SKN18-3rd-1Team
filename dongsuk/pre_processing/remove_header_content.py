...
def remove_content_before_first_header(file_path):
    """파일에서 첫 번째 # 제1편 헤더 이전의 모든 내용을 삭제합니다."""
    try:
        # 파일 읽기
        ...
        # 첫 번째 # 제1편 헤더 찾기 (다양한 형태 고려)
        first_header_match = re.search(r'^#\s*제\s*1\s*편', content, flags=re.MULTILINE)
        
        if first_header_match:
            # 첫 번째 # 제1편 헤더부터 끝까지의 내용만 유지
            ...
            print(f"✅ 제1편 이전 내용 삭제 완료: {file_path}")
            print(f"   - 삭제된 내용: {removed_lines}줄, {removed_chars}자")
            print(f"   - 백업 파일: {backup_path}")
            
            return True
        else:
            # # 제1편 헤더가 없으면 백업 파일 삭제
            os.remove(backup_path)
            print(f"ℹ️  # 제1편 헤더를 찾을 수 없음: {file_path}")
            return False
...
def process_all_final_files(base_path="data"):
    """지정된 경로에서 *_final.txt 파일들을 찾아서 제1편 이전 내용을 삭제합니다."""
    try:
...
def preview_header_content(file_path, preview_lines=10):
    """파일의 첫 번째 # 제1편 헤더 이전 내용을 미리보기합니다."""
    try:
        with open(file_path, 'r', encoding='utf-8') as file:
            content = file.read()
        ...
        if first_header_match:
            ...
            if before_header.strip():
                ...
                print(f"\n📋 {os.path.basename(file_path)} - 첫 번째 # 제1편 헤더 이전 내용:")
                print("-" * 60)
                ...
                print(f"\n✅ {os.path.basename(file_path)} - 첫 번째 # 제1편 헤더 이전에 내용이 없습니다.")
        else:
            print(f"\n❌ {os.path.basename(file_path)} - # 제1편 헤더를 찾을 수 없습니다.")
            
    except Exception as e:
        print(f"❌ 미리보기 오류 ({file_path}): {e}")

if __name__ == "__main__":
    print("🔧 제1편 이전 내용 삭제 도구")
    print("📋 # 제1편 헤더 이전의 모든 내용을 삭제합니다")
    print("=" * 50)
    print("1. 특정 파일 처리")
    ...
    if choice == "1":
        ...
        # 확인 메시지
        print("⚠️  이 작업은 모든 *_final.txt 파일에서 첫 번째 # 제1편 헤더 이전의 모든 내용을 삭제합니다.")
